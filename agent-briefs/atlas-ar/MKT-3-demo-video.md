# Agent brief — MKT-3 Demo video script

**Brief ID:** `MKT-3`  
**Title:** Product demo video scripts — Android (WebXR) + iOS (Quick Look)  
**Agent roles:** Video Optimization Specialist · Visual Storyteller · Content Creator · Product Manager  
**Sprint / Phase:** Phase 2 · NEXUS-Sprint **Batch 26** (proposed)  
**Priority:** P1  

### Context

Prospects need a **short, shareable product demo** showing real floor AR on phone — not slides. Android uses **Chrome WebXR** (Start AR, floor scan, AR/3D toggle). iOS uses **Safari Quick Look** (View in AR, USDZ). Scripts must match actual product flows in `main.ts` and admin help.

### Locked constraints

- **No app store** narrative — browser only  
- iOS: Quick Look only (no WebXR on iOS)  
- Android: Chrome floor AR + optional 3D inspect  
- Demo URL: `https://main.d3t9wmef56h86w.amplifyapp.com/demo`  
- Win theme aligned with SAL-1 / MKT copy  

### Inputs

- `docs/atlas-ar/PRD-v1.md` (FR-C2, FR-C3)  
- `src/ui/marketing-copy.ts`, `src/ui/admin-help-content.ts`  
- `docs/atlas-ar/SALES-DECK.md` slide 4  

### Deliverables

- [ ] Master doc: `docs/atlas-ar/MKT-3-DEMO-VIDEO-SCRIPT.md`  
- [ ] Android hero cut (~90s) + extended cut (~3 min) shot lists  
- [ ] iOS hero cut (~75s) shot list  
- [ ] VO script, on-screen text, B-roll checklist, YouTube packaging  

### Out of scope

- Video editing / render (production team)  
- Paid ad creative variants (separate MKT task)  

### Handoff

- Marketing / founder records on physical devices  
- Embed on landing page + sales deck slide 4 link  
- **User gate** — confirm Batch 26  
