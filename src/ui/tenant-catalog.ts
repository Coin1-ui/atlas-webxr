...
/**
 * Every shopper-facing line on this page has to match the AR path the phone will
 * actually take. iOS hands off to Safari AR, where there is no cyan placement
 * ring, so the Android tips would describe an interface the shopper never sees.
 * Caller passes `iosSafariAr` (see main.ts) and all five copy slots below branch
 * together — lead, tips, trust badge, requirements line, and CTA label.
 */
export function renderTenantCatalog(
  ...
  const arCta = arCtaLabel("catalog");
  const heroLead = iosAr
    ? `Tap <strong>${escapeHtml(arCta)}</strong> — Safari AR opens. Move your phone to find the floor, then tap to place.`
    : `Tap <strong>${escapeHtml(arCta)}</strong> to place any piece on your floor at true scale — no app install.`;
  const tips = iosAr
    ? ["Tap model → Safari AR opens", "Move phone slowly over the floor", "Tap screen to place"]
    : ["Cyan ring = placeable", "Red = blocked", "True floor scale"];
  const trustBadge = iosAr ? MKT.catalogTrust3 : MKT.catalogTrust2;
  const requirements = iosAr ? MKT.homeFooterIos : MKT.homeFooterAndroid;
  const heroBadges = `<ul class="catalog-hero-badges" aria-label="${iosAr ? "Safari AR tips" : "AR tips"}">
      ...
      <div class="catalog-shell">
        ...
        ${
          options.usageWarning
            ? `<div class="camera-warning catalog-usage-warning" role="status">${escapeHtml(options.usageWarning)}</div>`
            : ""
        }

        <section class="catalog-hero catalog-hero--v2">
          <div class="catalog-hero-copy">
            <span class="a-badge a-badge--accent catalog-trust-badge">${trustBadge}</span>
        ...
        ${
          ...
          ${catalog
            ...
            <article class="catalog-card catalog-card--v2" role="listitem" data-index="${index}">
              ...
              <div class="catalog-card-body">
                ...
                <button type="button" class="catalog-btn-ar catalog-btn-ar--v2" data-action="ar" data-index="${index}">${escapeHtml(arCta)}</button>
              </div>
            </article>
          `,
            )
          ...
          <p class="catalog-empty-body">Your team uploads 3D models from the desktop admin dashboard. Once the first model is live, <strong>${escapeHtml(arCta)}</strong> buttons appear here — shoppers open this link on Chrome or Safari with no app install.</p>
        ...
        <footer class="catalog-footer catalog-footer--v2">
          ...
          <p class="catalog-footer-note">${requirements}</p>
