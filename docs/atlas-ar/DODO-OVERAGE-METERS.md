...
5. **Overage-gated plan change (2026-07-25 / updated 2026-07-29):** Usage-hybrid only (no classic MONTHLY checkout). **Upgrade/Downgrade on hybrids → always remount checkout** (Dodo `change-plan` returns `500 INTERNAL_SERVER_ERROR` on usage products and leaves stale meters). **Stay put** → renews; meters bill overage when exceeded. Classic MONTHLY (if any) still uses scheduled `change-plan` when within limits. `billing/status.inOverage` / `planChangeMode` (`remount_checkout` when `usageHybrid`).
...
8. **Stuck payment cancel (2026-07-25):** If a Dodo subscription payment stays `processing` longer than **1 hour** (`ATLAS_STUCK_PAYMENT_HOURS`), Atlas **immediately** cancels the subscription (`status: cancelled`), expires entitlement (AR paused), and requires **Subscribe** again. EventBridge schedule + `billing/status` opportunistic check.
