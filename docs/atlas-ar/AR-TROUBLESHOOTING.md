...
**In-app mirror:** Desktop admin → **Help** (`/admin/help`) — HTTPS, camera, browser AR, and common fixes sections
...
| iPhone shows Start AR only | Use **View in AR** on catalog cards (Safari AR) |
...
Browsers expose camera and in-browser floor AR only on **secure contexts**:
...
- Corporate TLS inspection may break browser AR — test on cellular if store Wi‑Fi fails.
...
1. Shopper taps **Start AR** (direct tap required for browser AR).
...
- Catalog **View in AR** → **Safari AR** (USDZ generated at upload).
...
If Safari AR opens but model is missing: confirm USDZ generation completed in admin after GLB upload.

---

## Chrome browser AR vs Safari AR

| Platform | Path | UX |
|----------|------|-----|
| Android Chrome | Browser AR / floor AR | Start AR → floor scan → place → AR/3D toggle |
| iPhone Safari | Safari AR | View in AR → place in room |
